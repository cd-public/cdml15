...
\BEQ
p(\vv) = \sum_{h_k}\frac{\myexp{f(h_k,\vv)}}{Z}\sum_{\hh_{[-k]}} g(\hh_{[-k]},\vv)
\label{eq:marginal}
\EEQ
...
\BEQ
p(h_k|\vv) = \sigmoid{\prod_{h_j \neq h_k}h_j(b_j + \sum_i v_i\theta_{i,j})}
...
\BEAN
\nabla_\Psi A^t(\Psi) &=& \frac{\sum_{\hh^t} p(\hh^t,\vv^t) \nabla(-E(\hh^t,\vv^t))}{p(\vv^t)} \\
&=& \frac{1}{p(\vv^t)}\sum_{\hh^t} \frac{\myexp{-E(\hh^t,\vv^t)}\nabla(-E(\hh^t,\vv^t))}{\sum_{\hh'}\myexp{-E(\hh',\vv^t)}} \label{eq:Agrad}
\EEAN
...
&=& \sum_{\hh,\vv} \frac{\myexp{-E(\hh,\vv)}\nabla E(\hh,\vv)}{\sum_{\hh',\vv'}\myexp{-E(\hh',\vv')}} \label{eq:Bgrad}
\EEAS
Using Equations~\ref{eq:Erbm},\ref{eq:Agrad},\ref{eq:Bgrad} compute partial derivatives with respect to $\theta_{i,j}$,$a_i$, and $b_j$.

\BEAS
\partd{A^t(\Psi)}{\theta_{i,j}}  &=&   v_ih_j\sum_{\hh} p(\hh|\vv)  \\
\partd{A^t(\Psi)}{a_i} &=& v_i\sum_{\hh} p(\hh|\vv)  \\
\partd{A^t(\Psi)}{b_j} &=& h_j\sum_{\hh} p(\hh|\vv)  \\
\partd{B(\Psi)}{\theta_{i,j}}  &=& v_ih_j\sum_{\hh,\vv} p(\hh,\vv)  \\
\partd{B(\Psi)}{a_i}  &=& v_i\sum_{\hh,\vv} p(\hh,\vv) \\
\partd{B(\Psi)}{b_j}  &=& h_j\sum_{\hh,\vv} p(\hh,\vv)\\
\partd{\ALL(\Psi)}{\theta_{i,j}}  &=&  v_ih_j\sum_t \frac{1}{T}\left[\sum_{\hh} p(\hh|\vv) - \sum_{\hh,\vv} p(\hh,\vv) \right] \\
\partd{\ALL(\Psi)}{a_i}  &=&  v_i\sum_t \frac{1}{T}\left[\sum_{\hh} p(\hh|\vv) - \sum_{\hh,\vv} p(\hh,\vv)\right] \\
\partd{\ALL(\Psi)}{b_i}  &=&  h_j\sum_t \frac{1}{T}\left[\sum_{\hh} p(\hh|\vv) - \sum_{\hh,\vv} p(\hh,\vv) \right]
\EEAS
